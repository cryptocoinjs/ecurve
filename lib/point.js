...
ECPointFp.prototype.getX = function() {
  ...
  return this.curve.fromBigInteger(this.x.toBigInteger().multiply(this.zinv).mod(this.curve.p))
}
...
ECPointFp.prototype.getY = function() {
  ...
  return this.curve.fromBigInteger(this.y.toBigInteger().multiply(this.zinv).mod(this.curve.p))
}
...
ECPointFp.prototype.equals = function(other) {
  ...
  // u = Y2 * Z1 - Y1 * Z2
  var u = other.y.toBigInteger().multiply(this.z).subtract(this.y.toBigInteger().multiply(other.z)).mod(this.curve.p)

  if (u.signum() !== 0) return false

  // v = X2 * Z1 - X1 * Z2
  var v = other.x.toBigInteger().multiply(this.z).subtract(this.x.toBigInteger().multiply(other.z)).mod(this.curve.p)
...
ECPointFp.prototype.negate = function() {
  var y = this.y.toBigInteger().negate().mod(this.curve.p)
  return new ECPointFp(this.curve, this.x, this.curve.fromBigInteger(y), this.z)
}
