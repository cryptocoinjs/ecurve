...
ECFieldElementFp.prototype.subMod = function(a, b) {
  var mA = this.modQ(a);
  var mB = this.modQ(b);
  if (BigMath.compare(mA, mB) < 0) {
    // A is smaller than B
    return BigMath.subtract(BigMath.add(mA, this.q), mB);
  }
  return BigMath.subtract(mA, mB);
}
...
ECFieldElementFp.prototype.negate = function() {
  // -x == q-x
  return new ECFieldElementFp(this.q, this.subMod(this.q, this.x));
};
...
ECFieldElementFp.prototype.subtract = function(b) {
  return new ECFieldElementFp(this.q, this.subMod(this.x, b.toBigInteger()));
};
...
// Subtract b from a, mod Q
ECPointFp.prototype.subMod = function(a, b) {
  var mA = this.modQ(a);
  var mB = this.modQ(b);
  if (BigMath.compare(mA, mB) < 0) {
    // A is smaller than B
    return BigMath.subtract(BigMath.add(mA, this.curve.q), mB);
  }
  return BigMath.subtract(mA, mB);
}
...
ECPointFp.prototype.equals = function(other) {
  ...
  var u, v;
  // u = Y2 * Z1 - Y1 * Z2
  u = this.subMod(BigMath.multiply(other.y.toBigInteger(), this.z), BigMath.multiply(this.y.toBigInteger(), other.z));
  if(!BigMath.isZero(u)) return false;
  // v = X2 * Z1 - X1 * Z2
  v = this.subMod(BigMath.multiply(other.x.toBigInteger(), this.z), BigMath.multiply(this.x.toBigInteger(), other.z));
...
ECPointFp.prototype.add = function(b) {
  ...
  // u = Y2 * Z1 - Y1 * Z2
  var u = this.subMod(BigMath.multiply(b.y.toBigInteger(), this.z), BigMath.multiply(this.y.toBigInteger(), b.z));
  // v = X2 * Z1 - X1 * Z2
  var v = this.subMod(BigMath.multiply(b.x.toBigInteger(), this.z), BigMath.multiply(this.x.toBigInteger(), b.z));
  ...
  var v2 = this.modQ(BigMath.square(v));
  var v3 = this.modQ(BigMath.multiply(v2, v));
  var u2 = this.modQ(BigMath.square(u));
  var u3 = this.modQ(BigMath.multiply(u2, u));

  // x3 = v * (z2 * (z1 * u^2 - 2 * x1 * v^2) - v^3)
  var r1 = this.subMod(BigMath.multiply(z1, u2), BigMath.multiply(BigMath.shift(x1, 1), v2));
  var r2 = this.subMod(BigMath.multiply(z2, r1), v3);
  var x3 = this.modQ(BigMath.multiply(v, r2));
  // y3 = z2 * (3 * x1 * u * v^2 - y1 * v^3 - z1 * u^3) + u * v^3
  var r1 = BigMath.multiply(BigMath.multiply(BigMath.multiply(THREE, x1), u), v2);
  var r2 = this.subMod(this.subMod(r1, BigMath.multiply(y1, v3)), BigMath.multiply(z1, u3));
...
ECPointFp.prototype.twice = function() {
  ...
  console.log(x1,y1,z1,a);

  var y1sqz1 = this.modQ(BigMath.multiply(BigMath.square(y1), z1));

  // w = 3 * x1^2 + a * z1^2
  var w = this.modQ(BigMath.multiply(THREE, BigMath.square(x1)));
  if(!BigMath.compare(a, ZERO_BUFFER)) {
    w = this.modQ(BigMath.add(w, BigMath.multiply(a, BigMath.square(z1))));
  }
  console.log('w:',w);
  // x3 = 2 * y1 * z1 * (w^2 - 8 * x1 * y1^2 * z1)
  var r1 = this.modQ(BigMath.multiply(BigMath.shift(x1, 3), y1sqz1));
  console.log('r1:',r1);
  var r2 = this.subMod(BigMath.square(w), r1);
  console.log('r2:',r2);
  var r3 = this.modQ(BigMath.multiply(BigMath.shift(y1, 1), z1));
  console.log('r3:',r3);
  var x3 = this.modQ(BigMath.multiply(r2, r3));
  console.log('x3:',x3);

  // y3 = 4 * y1^2 * z1 * (3 * w * x1 - 2 * y1^2 * z1) - w^3
  var r1 = this.modQ(BigMath.multiply(BigMath.multiply(THREE, w), x1));
  console.log('r1:',r1);
  var r2 = BigMath.shift(y1sqz1, 1);
  console.log('r2:',r2);
  var r3 = this.modQ(BigMath.multiply(BigMath.shift(y1sqz1, 2), this.subMod(r1, r2))); //!
  console.log('r3:',r3);
  var r4 = this.modQ(BigMath.multiply(BigMath.square(w), w));
  console.log('r4:',r4);
  var y3 = this.subMod(r3, r4);
  console.log('y3:',y3);

  // z3 = 8 * (y1 * z1)^3
  var r1 = this.modQ(BigMath.multiply(y1, z1));
  console.log('r1:',r1);
  var r2 = this.modQ(BigMath.multiply(BigMath.square(r1), r1));
  console.log('r2:',r2);
  var z3 = this.modQ(BigMath.shift(r2, 3));
  console.log('z3:',z3);
...
ECPointFp.prototype.multiply = function(k) {
  ...
  for(i = BigMath.mostSignificantBit(h) - 2; i > 0; --i) {
    ...
    console.log('Byte:',byteNum, 'Bit:',bitNum, 'Test:',testNum, 'h:',hBit, 'e:',eBit);
...
ECCurveFp.prototype.decodePointHex = function(s) {
  ...
  return new ECPointFp(this,
       this.fromBigInteger(flipBuf(new Buffer(xHex, 'hex'))),
       this.fromBigInteger(flipBuf(new Buffer(yHex, 'hex'))));
